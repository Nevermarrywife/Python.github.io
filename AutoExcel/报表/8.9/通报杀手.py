...
countys2 = ['盐湖区', '芮城县', '平陆县', '临猗县', '万荣县', '河津市', '稷山县', '垣曲县', '绛县', '闻喜县', '新绛县','永济市','夏县']
...
active_v = ('JT2130-运城二季度和飞信新增活动（3个月）', 
            'JT2120-运城二季度和飞信新增活动（4个月）',
            'JT2121-运城二季度和飞信新增活动（6个月）',
            'JT2130-运城二季度和飞信新增活动（3个月）|新增5元和飞信享7折优惠，合约3个月',
            'JT2120-运城二季度和飞信新增活动（4个月）|新增5元和飞信享7折优惠，合约4个月', 
            'JT2121-运城二季度和飞信新增活动（6个月）|新增5元和飞信享7折优惠，合约6个月',
            'JT2139-运城集团V网新增合约（6个月）|V网合约6个月',
            'JT2138-运城集团V网新增活动（临猗）|运城果都小号合约3个月','JT2140-运城集团V网新增合约（12个月）|V网合约12个月',
            'JT2137-运城集团V网新增活动（盐湖）|运城关公小号合约3个月','JT2137-运城集团V网新增活动（盐湖）',
...
for i in data_v:
    if i['work'] == '增加':
        netv.append(i['county'])
        if i['county'] == '永济':
            netv_yj.append(i['channelname'])
            netv_yj.append(i['person'])
...
wb_tb = app.books.open('8月通报专用.xlsx')
sht_tb_yj = wb_tb.sheets['永济通报']
sht_tb_qx = wb_tb.sheets['区县通报']

for i in range(2, len(persons) + 2):    #永济
    ...
    sht_tb_yj.range((i, 18)).value = active2.count(persons[i - 2])
    sht_tb_yj.range((i, 20)).value = active1.count(persons[i - 2])
for i in range(2, len(periods) + 2):  # 写入永济和彩云
    sht_tb_yj.range((i, 2)).value = periods[i - 2]['hcy']
for i in range(2, len(countys) + 2):    #区县
    sht_tb_qx.range((i, 10)).value = yingjian.count(countys[i - 2])
    sht_tb_qx.range((i, 2)).value = fanghao.count(countys_yc[i - 2])
    sht_tb_qx.range((i, 12)).value = netv.count(countys[i - 2])
    sht_tb_qx.range((i, 4)).value = kuandai.count(countys2[i - 2])
    sht_tb_qx.range((i, 6)).value = iptv.count(countys2[i - 2])
print("数据写入成功!")
